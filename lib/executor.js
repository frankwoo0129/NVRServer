...
var executor = function(address) {
	...
	event.on('start', function() {
		...
		var inputcommand = [
			...
			'-i', self.stream,
			'-loglevel', 'error'
			//'-filter_complex', '\'[0:v]yadif,split=2[out1][out2]\''
			]; 
		var videocommand = [
			'-map', 0,
			'-vcodec', 'copy',
		...
		var jpegcommand = [
			...
			'-segment_time', 2,
			'-segment_list_size', 2,
			'-segment_list', self.temp+'/out.m3u8',
			'-strftime', 1,
			self.temp+'/%M%S.ts'];
		...
		check.all('onVideoDir', 'onJPEGDir', 'onTempDir', function(video, jpeg, temp) {
			if (!video) {
				...
				throw new Error('tempdir error, '+self.temp);
			} else {
		...
		fs.exists(self.temp, function(exists) {
			if (!exists)
				mkdirp(self.temp, function (err) {
					if (err) {
