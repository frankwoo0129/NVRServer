	...
	underscore.each(configsource, function (config) {
		if (!config.address) {
			...
			if (!config.user || !config.passwd) {
				camera.highstream = util.format(stream.high.replace('%s:%s@%s', '%s'), config.address);
				camera.lowstream = util.format(stream.low.replace('%s:%s@%s', '%s'), config.address);
			} else {
				camera.highstream = util.format(stream.high, config.user, config.passwd, config.address);
				camera.lowstream = util.format(stream.low, config.user, config.passwd, config.address);
			}
